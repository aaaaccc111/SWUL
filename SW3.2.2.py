...
def update_tray_title(new_title):
    global tray_icon
    if tray_icon:
        tray_icon.title = new_title

def handle_detected_software(pname):
    global tray_icon
    if tray_icon:
        update_tray_title(f"軟體使用紀錄 - {user_name} ({pname})")

def handle_software_closed():
    global tray_icon
    if tray_icon:
        update_tray_title(f"軟體使用紀錄 - {user_name}")
            ...
            for pid, pname, user_name in processes:
                try:
                    ...
                    if pid not in start_times and is_user_initiated(psutil.Process(pid)):
                        ...
                        handle_detected_software(pname)

                    # # 如果PID已記錄，更新其 CPU 和記憶體使用率
